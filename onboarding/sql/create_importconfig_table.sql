...
CREATE TABLE IF NOT EXISTS dba."timportconfig" (
    ...
    -- For 'filename': regex pattern (e.g., '\d{4}\d{2}\d{2}')
    ...
    CONSTRAINT valid_date CHECK (
        (DateConfig = 'filename' AND DateLocation ~ '.*\d.*')
...
COMMENT ON TABLE dba."timportconfig" IS 'Configuration table for importing flat files (CSV, XLS, XLSX) into the database, specifying file patterns, directories, metadata, and date extraction rules.';
