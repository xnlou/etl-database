-- Drop all existing versions of insert_timportconfig and update_timportconfig procedures to eliminate duplicates
DO $$
DECLARE
    proc_record RECORD;
BEGIN
    -- Drop all procedures named insert_timportconfig or update_timportconfig in the dba schema
    FOR proc_record IN (
        ...
        WHERE nspname = 'dba'
        AND proname IN ('insert_timportconfig', 'update_timportconfig')
    ) LOOP
    ...
    EXECUTE 'DROP PROCEDURE IF EXISTS dba.insert_timportconfig(VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, BIT) CASCADE';
    EXECUTE 'DROP PROCEDURE IF EXISTS dba.update_timportconfig(INT, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, BIT) CASCADE';
...
COMMENT ON TABLE dba."timportconfig" IS 'Configuration table for importing flat files (CSV, XLS, XLSX) into the database, specifying file patterns, directories, metadata, date extraction rules, and delimiter for parsing filenames.';
...
BEGIN
    IF NOT EXISTS (
        ...
        AND routine_name = 'insert_timportconfig'
    ) THEN
    ...
    IF NOT EXISTS (
        ...
        WHERE grantee = 'etl_user'
        AND routine_schema = 'dba'
        AND routine_name = 'update_timportconfig'
    ) THEN
